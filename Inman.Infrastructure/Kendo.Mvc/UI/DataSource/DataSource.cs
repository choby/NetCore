...
namespace Kendo.Mvc.UI
{
    public class DataSource : JsonObject
    {
        ...
        {
            if (data is IEnumerable<AggregateFunctionsGroup>)
            {
                if (!ServerGrouping)
                {
        ...
        private void MergeAggregateTypes(DataSourceRequest request)
        {
            if (Aggregates.Any())
            {
                ...
                {
                    var match = Aggregates.SingleOrDefault(agg => agg.Member.Equals(requestAggregate.Member, StringComparison.OrdinalIgnoreCase));
                    if (match != null)
                    {
