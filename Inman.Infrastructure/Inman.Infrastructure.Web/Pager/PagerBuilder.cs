...
namespace Inman.Infrastructure.Web
{
    internal class PagerBuilder
    {
        ...
        private const string GoToUrlScript = "function _MvcPager_GoToUrl(_psd){ location.href=document.getElementById(_psd).value; }";
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            if (pageIndex > _totalPageCount || _pageIndex == pageIndex)
                return null;
        ...
        private string GenerateUrl(int pageIndex, int pageSize)
        {
            //return null if  page index larger than total page count or page index is current page index
            ...
            routeValues[_pagerOptions.PageSizeParameterName] = pageSize;
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            else
            {
                foreach (PagerItem item in pagerItems)
                {
                    if (!_pagerOptions.IsPost)
                        ...
                        sb.Append(GeneratePagerElementPost(item));
                }
            }
            ...
            if (_pagerOptions.ShowPageSizeDropDownList)
            {
                sb.Append(BuildPageSizeDropDownListSection(ref pagerScript));
            }

            if (_pagerOptions.ShowPagingInfo)
            {
                sb.Append(BuildPagingInfoSection());
            }
            ...
            if (_pagerOptions.IsPost)
            {
                string strInnerScript = "document.forms['" + _pagerOptions.FormName + "'].action=href;document.forms['" + _pagerOptions.FormName + "'].submit();";

                if (string.IsNullOrWhiteSpace(_pagerOptions.FormName))
                {
                    strInnerScript = "$('.pagerItem').parents('form').attr('action',href);$('.pagerItem').parents('form').submit();";
                }

                //通过 _pagerOptions.FormName获得用户选定表单名称，修改其 action，然后提交。
                pagerScript += " function _PagePostSubmit(href){" + strInnerScript + " }";
        ...
        private string BuildPageSizeDropDownListSection(ref string pagerScript)
        {
            pagerScript += GoToUrlScript;
            ...
            string controlId = "_MvcPager_Ctrl" + ctrlIndex;
            string onChangeScript = " onchange=\"_MvcPager_GoToUrl({0})\"";

            var piBuilder = new StringBuilder();
            piBuilder.AppendFormat("<select id=\"{0}\"{1} style=\"width:70px\">", controlId + "_psd", string.Format(onChangeScript, "this.id"));
            List<int> arrPageSize = new List<int>() { 20, 50, 100 };

            if (_pagerOptions.PageSizeList != null && _pagerOptions.PageSizeList.Count > 0)
            {
                arrPageSize = _pagerOptions.PageSizeList;
            }
            if (!arrPageSize.Contains(PageSize))
            {
                arrPageSize.Add(PageSize);
                arrPageSize.Sort();
            }
            for (int i = 0; i < arrPageSize.Count; i++)
            {
                piBuilder.AppendFormat("<option value=\"{0}\"", GenerateUrl(1, arrPageSize[i]));
                if (arrPageSize[i] == PageSize)
                {
                    piBuilder.Append(" selected=\"selected\"");
                }
                piBuilder.AppendFormat(">{0}</option>", arrPageSize[i]);
            }
            piBuilder.Append("</select>");
        ...
        private string BuildPagingInfoSection()
        {
            if (TotalCount < 0)
                return string.Empty;
            ...
            int intFirstRowIndex = (_pageIndex - 1) * PageSize + 1;
            int intLastRowIndex;
            if (_pageIndex == _totalPageCount)//最后一页
            {
                intLastRowIndex = intFirstRowIndex - 1 + TotalCount % PageSize;
            }
            else
            {
                intLastRowIndex = intFirstRowIndex - 1 + PageSize;
            }

            return string.Format("<span class=\"pagingInfo\">Displaying {0} to {1} of {2} ({3} Pages)</span>", intFirstRowIndex, intLastRowIndex, TotalCount, _totalPageCount);
        }
        ...
        #region POST分页
        private MvcHtmlString GeneratePagerElementPost(PagerItem item)
        {
            //pager item link
            string url = GenerateUrl(item.PageIndex);
            if (item.Disabled) //first,last,next or previous page
            ...
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? _html.Encode(item.Text)
                                                 : String.Format("<a class='pagerItem' href='javascript:_PagePostSubmit (\"{0}\")'>{1}</a>", url, item.Text));
        }
