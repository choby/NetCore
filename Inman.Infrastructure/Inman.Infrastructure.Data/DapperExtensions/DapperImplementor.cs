...
using System.Text;
using Dapper;
using Inman.Infrastructure.Data.DapperExtensions.Mapper;
using Inman.Infrastructure.Data.DapperExtensions.Sql;
using System.Data.Common;

namespace Inman.Infrastructure.Data.DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        IEnumerable<TField> GetFieldList<T, TField>(IDbConnection connection, string selectField, object predicate, IList<ISort> sort, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class;
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        public IEnumerable<TField> GetFieldList<T, TField>(IDbConnection connection, string selectField, object predicate, IList<ISort> sort, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
            IPredicate wherePredicate = GetPredicate(classMap, predicate);
            return GetFieldList<T, TField>(connection, classMap, selectField, wherePredicate, sort, transaction, commandTimeout, true);
        }
        ...
        protected IEnumerable<TField> GetFieldList<T, TField>(IDbConnection connection, IClassMapper classMap, string selectField, IPredicate predicate, IList<ISort> sort, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            string sql = SqlGenerator.Select(classMap, selectField, predicate, sort, parameters);
            DynamicParameters dynamicParameters = new DynamicParameters();
            foreach (var parameter in parameters)
            {
                dynamicParameters.Add(parameter.Key, parameter.Value);
            ...
            return connection.Query<TField>(sql, dynamicParameters, transaction, buffered, commandTimeout, CommandType.Text);
        }
