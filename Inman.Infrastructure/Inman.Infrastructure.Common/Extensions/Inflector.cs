                ...
                for(int i = 0; i < words.Length; i++)
                {
                    if(words[i].Length > 0)
                    {
                        ...
                        if(restOfWord.IsUpperCase())
                            restOfWord = restOfWord.ToLower(); //to core modify:CultureInfo.CurrentUICulture

                        char firstChar = char.ToUpper(word[0]);//to core modify:, CultureInfo.CurrentUICulture
                        words[i] = String.Concat(firstChar, restOfWord);
                    }
                }
                return String.Join(joinString, words);
            }
            return String.Concat(words[0].Substring(0, 1).ToUpper(), words[0].Substring(1));//to core modify：CultureInfo.CurrentUICulture
        }
